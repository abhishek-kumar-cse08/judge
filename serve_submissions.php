...
<?php
    ...
    function check_query( $code, $user, $type, $offset, $ranges ){
            $con = start_connection();
            $res = mysql_query( 'SELECT * FROM questions WHERE code = "'.$code.'" AND question_time <= '.time(), $con );
            ...
            if( $type == "all" )
                ...
                $res = mysql_query( 'SELECT * FROM submissions WHERE question = "'.$code.'" AND user = "'.$user.'" LIMIT '.$offset.',  '.$ranges, $con );
            else
	...
	if( !$code || !$type || !$ranges )
        ...
        $ret = check_query( $code, $_SESSION["username"], $type, $offset, $ranges ) ;
        if( strstr( $ret, "-1") )
            echo $ret;
        else
