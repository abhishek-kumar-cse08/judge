...
<script>
	...
	function download( question, filename ){
		window.open( "download_submissions.php?question="+question+"&filename="+filename );
	}
	...
	function get_records( code, type, direction ){
		...
		for( i=0; response[i]; i++ ){
			inHtml += "<tr><td align='center'><input type='button' value='" + response[i]['id'] + "' onclick=\"download('" + response[i]['question'] +
			"', '"+ response[i]['file'] + "')\" class='button_small'>" + "</input></td><td align='center'><a href='question.php?code=" + 
			response[i]['question'] + "'>" + response[i]['question'] + "</a></td><td align='center'>" + response[i]['user']+"</td><td align='center'>" + 
	...
	xmlhttp.open( "GET", "serve_submissions.php?code="+code+"&type="+type+"&offset="+offset+"&ranges="+ranges, true );
	xmlhttp.send(  );
